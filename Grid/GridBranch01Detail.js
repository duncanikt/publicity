  ...
  var url =
    'https://duncanikt.ddns.net/detail/detaildata?search=' +
  ...
  function toggleButton(button) {
    ...
    } else 
    {
      ...
      "weight"
        ...
        data = data02;
        // 發送修改請求，保存資料
        var url = 'https://duncanikt.ddns.net/detail/detailupdate';
        fetch(url, {
          method: 'POST',
          headers: {
  ...
  function getMaxValueFromDatabase(Master) 
    {
      // 在這裡訪問並使用全域變數 Master 的值來自function search
        ...
        var url = 'https://duncanikt.ddns.net/detail/detailmaxValue?master=' + encodeURIComponent(currentMaster);
        fetch(url)
          .then(function(response) {
            return response.json();
          })
  ...
  function saveRecord() 
    {
      ...
      fetch('https://duncanikt.ddns.net/detail/detailinsert', { // 替換為後端 MongoDB 的 URL
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    function deleteRecord(button, ABC1, ShoulderWidth1) {
      ...
      // 發送刪除請求
      var url = 'https://duncanikt.ddns.net/detail/detaildelete';
